...
class LossComputer:
    ...
    def display(self, split):   ### WARNING: again, the argument was changed from bool type train to string value split.
        ...
        if split == 'train':
            ...
            for i in range(self.n_groups):
                group_acc.append(self.group_correct[i] / self.group_total[i])
                group_repr.append(self.get_group_repr_fn(i))
            return f"{'VAL' if split == 'valid' else 'TEST'} {acc:.4f} group " + "|".join([f"{group_repr[i]} {group_acc[i]:.4f}" for i in range(self.n_groups)])
        else:
