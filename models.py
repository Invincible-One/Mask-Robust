...
def get_model(model, pretrained, n_classes):
    '''
    Comment:
        The original implementation does not consider using pretrained weights. Therefore when adding the new feature, not only the handcrated VGG is not useful, but I only add the feature on resnet50, since this is the only model we are using.
    '''
    if model == "resnet50":
        if pretrained:
