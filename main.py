...
def get_args():
    ...
    ##basic settings
    parser.add_argument("--method", choices={"ERM", "JTT", "AUG", "AUG_TT", "AUG_JTT", "JTT_AUG"}, required=True)
    ...
    parser.add_argument("--use_pretrained", action="store_true", help="use pretrained weights")   ###Comment: A new feature

    parser.add_argument("--root", type=str, default="/scratch/ym2380/data/")
    parser.add_argument("--n_workers", type=int, default=6)
    parser.add_argument("--train_fraction", type=float, default=1.0)
    parser.add_argument("--run_test", action="store_true", help="evaluate on the test set for each epoch")   ###Comment: A new feature
...
def check_args(args):
    ### argparse with nargs=='*' seems inconsistent (could either be None or [] if not passing value).
    ### I add this code to avoid None being non-iterable. I can also add default=[] in or write a special
    ### function for parser, yet those could be unsafe or is a bit complicated.
    if args.res_epochs == None:
        args.res_epochs = []
    if args.save_model_epochs == None:
    ...
    if args.method == 'ERM':
        ...
        pass
    elif args.method == "AUG_TT":
        pass
    elif args.method == "JTT_AUG":
        pass
    elif args.method == "AUG_JTT":
        pass
    else:
...
if __name__ == "__main__":
    ...
    ###save model settings
    if args.save_model_epochs:   ###Added conditional sentences because it would cause conflicts when creating directories, when arries of tasks running
    ...
    if args.run_test:   ### WARNING: new feature of printing out the test evaluation for epochs
        test_set = get_data(args, 'test')   ### WARNING: NO PEEKING!
    ...
    if args.run_test:
        test_loader = get_loader(dataset=test_set, train=False, args=args, **loader_kwargs)

    n_classes = train_set.n_classes
    n_groups = train_set.n_groups

    #Model Section
    network = get_model(model=args.model, pretrained=args.use_pretrained, n_classes=n_classes).to(device)
    ...
    for _ in range(args.n_epochs):
        ...
        train_log = run_epoch(
                ...
                optimizer=optimizer,
                split='train',   ### WARNING: changed from bool type train to string value split. Only takes from ('train', 'valid', 'test', 'all') as usual.
        ...
        valid_log = run_epoch(
                ...
                loss_computer=loss_computer,
                optimizer=optimizer,
                split='valid',   ### WARNING: changed from bool type train to string value split. Only takes from ('train', 'valid', 'test', 'all') as usual.
                )
        ...
        if args.run_test:
            test_log = run_epoch(
                    ...
                    loss_computer=loss_computer,
                    optimizer=optimizer,
                    split='test',    ### WARNING: changed from bool type train to string value split. Only takes from ('train', 'valid', 'test', 'all') as usual.
                    )
        
        runtime = time.time() - beg
        print(f"Epoch [{count:02d}/{args.n_epochs}] {progress_bar(count, args.n_epochs)}, Time {runtime:.0f}, {train_log}, {valid_log}{f', {test_log}' if args.run_test else ''}.")
        sys.stdout.flush()
